...
)
{
    ...
    if (!msg->gnss_fix_ok || !msg->rel_pos_valid) {
        RCLCPP_WARN_THROTTLE(
            ...
            "RELPOSNED not valid yet (gnss_fix_ok=%d, rel_pos_valid=%d); skipping.",
            static_cast<int>(msg->gnss_fix_ok),
            static_cast<int>(msg->rel_pos_valid)
        );
        return;
    }
        ...
        ublox_ubx_msgs::msg::CarrSoln::CARRIER_SOLUTION_PHASE_WITH_FIXED_AMBIGUITIES)
    {
        ...
        RCLCPP_WARN_THROTTLE(
            this->get_logger(), *this->get_clock(), 5000,
            "RTK not fixed (carr_soln=%d); heading unreliable, skipping.",
            static_cast<int>(carr_soln)
        );
        return;
    }
    ...
    if (rel_n == 0.0 && rel_e == 0.0) {
        RCLCPP_WARN(this->get_logger(),
                    "RELPOSNED baseline is zero; cannot compute heading.");
        return;
    }
    ...
    if (std::fabs(baseline_len - kExpectedBaselineM) > kBaselineToleranceM) {
        RCLCPP_WARN_THROTTLE(
            this->get_logger(), *this->get_clock(), 5000,
            "Baseline length %.3f m differs from expected %.2f m; "
            "possible wrong ambiguity fix, skipping.",
            baseline_len, kExpectedBaselineM
        );
        return;
    }
    ...
    RCLCPP_INFO(
        this->get_logger(),
        "Heading: %s, angle: %.2f deg, baseline heading: %.2f deg, "
        "rel_n: %.3f m, rel_e: %.3f m, len: %.3f m",
        heading_dir.c_str(),
        heading_angle,
        baseline_heading,
        rel_n,
